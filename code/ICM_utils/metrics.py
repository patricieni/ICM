...
def gower_distances(X, Y=None, feature_weight=None, categorical_features=None):
    ...
    Notes
    ------
    The non-numeric features, and numeric feature ranges are determined from
    X and not Y.
    ...
    # this if is necessary as strangelly the validator is rejecting numeric
    # arrays with NaN
    ...
    if np.issubdtype(categorical_features.dtype, np.int):
        ...
        categorical_features = np.array(categorical_features, dtype=np.bool)
    ...
    if not y_none:
        ...
        # This is to normalize the numeric values between 0 and 1.
        Y_num = np.divide(Y_num, max_of_numeric, out=np.zeros_like(Y_num),
                          where=max_of_numeric != 0)
    else:
        Y_cat = X_cat
        Y_num = X_num
    ...
    for i in range(n_rows):
        ...
        result = _gower_distance_row(X_cat[i, :], X_num[i, :],
                                     Y_cat[j_start:n_rows, :],
                                     Y_num[j_start:n_rows, :],
                                     feature_weight_cat,
                                     feature_weight_num,
...
def _gower_distance_row(xi_cat, xi_num, xj_cat, xj_num, feature_weight_cat,
                        feature_weight_num, feature_weight_sum,
                        categorical_features, ranges_of_numeric,
                        max_of_numeric):
    """Computes the gower distances between a set of rows of X and Y
    """
    # categorical columns
    sij_cat = np.where(xi_cat == xj_cat, np.zeros_like(xi_cat),
                       np.ones_like(xi_cat))
